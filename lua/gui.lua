...
gui.btn_connect.action=errutil.wrap(function(self)
	if con:is_connected() then
		con:disconnect()
	else
		con:connect()
...
function gui.inputtext:k_any(k)
	...
	elseif k == iup.K_DOWN then
		gui.inputtext.value = cmd_history:next()
	end
end
...
function gui.btn_exec:action()
	printf('> %s\n',gui.inputtext.value)
	cmd_history:add(gui.inputtext.value)
	...
	if prefs.gui_exec_direct and (string.find(gui.inputtext.value,'[%c%s]*^!') or string.find(gui.inputtext.value,'^[%c%s]*exec[%c%s]+')) then
		add_status(cli:execute(gui.inputtext.value))
	else
		if gui.cli_thread_status == 'readline' then
			local s
			s,gui.cli_thread_status=coroutine.resume(gui.cli_thread,gui.inputtext.value)
		else
