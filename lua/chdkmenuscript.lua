...
--[[
old style params are over multiple ines in abitrary order
...
function item_methods:finalize_type()
	...
	if not item_type_methods[self.paramtype] then
		errlib.throw{etype='internal_error',msg=('%s invalid item type %s'):format(name,self.paramtype)}
	end
...
function item_methods:format_shorthand()
	...
	local r=('#%s=%s %s%s%s'):format(self.name,self.val,qchar,self.desc,qchar)
...
@param %s %s
@default %s %s
]]):format(self.name,self.desc,self.name,self.val)
	if self.range then
...
chdk users strtol which (if standard) accepts 0x<hex> 0<octal> or dec
...
function header_methods:parse(str)
	-- emtpy allows line numbers to match
	for lnum,line in ipairs(util.string_split(str,'[\r\n]',{empty=true})) do
	...
	for i, item in ipairs(self.items) do
		...
		item:finalize_type()
		if item.deftype == '@' then
			if item.defval == nil then
				self:warnf('items %s missing default',item.name)
				item.defval = 0
				item.val = 0
			end
			if not item.desc then
				self:warnf('items %s missing description',item.name)
				item.desc = ''
			end
		end
		item:validate_current_value()
...
--              documenation to appear in the template without appearing in output
...
and optionally saving the process script and mergned settings
opts={
