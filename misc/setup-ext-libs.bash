...
General options:
 ...
 -gui: force above if platform defaults off (OSX)
 -redl: re-download files even if they exist

Options controlling which actions are performed:
 -nodl: skip downloads
 -nosha: do not check sha sums of downloaded files
...
init_vars() {
	...
	if [ "$opt_lua_ver" == "52" ] ; then
		...
		error_exit "Unsupported lua-ver $opt_lua_ver"
	fi
	...
	TEC_PKG_LINVER64="415"
# 32 bit libs no longer supplied
#	TEC_PKG_LINVER32="32"
	...
	SHASUM_FILE="${CHDKPTP_DIR}/misc/ext-libs-sha1.txt"
	shasum_cmd="sha1sum"
	if [ -z "$nosha" ] ; then
...
init_os() {
	...
	Windows)
		if [ "$BUILD_ARCH" == 'i686' ] ; then
			TEC_LIB_SFX="Win32_mingw6_lib.zip"
		elif [ "$BUILD_ARCH" == 'x86_64' ] ; then
			TEC_LIB_SFX="Win64_mingw6_lib.zip"
		else
	...
	Linux)
		...
		;;
		armv[67]l)
			# newer pis are arvmv7l, including the 64 bit (under raspbian, at least)
			TEC_SOURCE_BUILD=1
			# for bin names
...
do_shasum() {
	...
	local fname="$1"
	local hashline=`grep "[ *]$fname"'$' $SHASUM_FILE`
	if [ ! -z "$hashline" ] ; then
		if [ -z "$pretend" ] ; then
			echo "$hashline" | $shasum_cmd -c || error_exit "shasum check failed $fname"
		else
			echo "echo $hashline | $shasum_cmd -c "
		fi
	else
		warn "missing shasum for $fname"
	fi
}
...
do_all_shasum() {
	...
	if [ "$TEC_SOURCE_BUILD" == '1' ] ; then
		if [ -z "$nogui" ] ; then
			do_shasum "${CD_SRC_PKG}"
			do_shasum "${IUP_SRC_PKG}"
			# OSX needs freetype
			if [ ! -z "$USE_FREETYPE_SRC" ] ; then
				do_shasum "${FREETYPE_SRC_PKG}"
			fi
		fi
	else
		if [ "$BUILD_OS" == 'Windows' ] ; then
			do_shasum "${LIBUSBWIN32_PKG}"
		fi
		if [ -z "$nogui" ] ; then
			do_shasum "${CD_PKG}"
			do_shasum "${CD_LUA_PKG}"
			do_shasum "${IUP_PKG}"
			do_shasum "${IUP_LUA_PKG}"
		fi
	fi
...
prepare_source() {
# not required for current tecmake, old versions did not detect armv6l correctly
...
copy_built_lua() {
	...
	# copy doc dir so it will end up in build libs package
	create_dir "$BUILT_DIR"/"$LUA_VER_DIR"/doc
	do_cp doc/* "$BUILT_DIR"/"$LUA_VER_DIR"/doc
...
copy_runtime() {
	...
	local liblist
	if [ "$BUILD_OS" == 'Linux' ] ; then
		 liblist="$(cat "${CHDKPTP_DIR}"/misc/liblist-linux-lua"$LUA_VER_SFX".txt)"
	elif [ "$BUILD_OS" == 'Darwin' ] ; then
		 liblist="$(cat "${CHDKPTP_DIR}"/misc/liblist-osx-lua"$LUA_VER_SFX".txt)"
	fi
